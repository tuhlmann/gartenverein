...
object FeedbackScreen extends BootstrapCssBoundLiftScreen {
  ...
  val name = field("Name", "", trim, valMinLen(5, "Bitte geben Sie min. 5 Stellen Ihres Namens an"), ftrans())
  val email = field("Email Adresse", "", trim, valRegex(emailPattern, "Bitte geben Sie eine Emailadresse an"), ftrans())
  val message = textarea("Mitteilung", "", "placeholder" -> "Bitte geben Sie Ihre Mitteilung ein",
      trim, valMinLen(5, "Bitte geben Sie eine Mitteilung ein."), ftrans())

  override def finishButton: Elem = <button>Absenden</button>

  // Validation

  def formName = "feedbackFormContent"

  def finish() {
    MailSender.sendFeedback(name.is, email.is, message.is)
    S.notice("Vielen Dank! Ihre Mitteilung wurde versendet.")
