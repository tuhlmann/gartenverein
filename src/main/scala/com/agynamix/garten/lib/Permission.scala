  ...
  val entities: Set[String] = Set(Permission.wildcardToken))
{
  def implies(p: Permission) = {
    //println(this.toString()+" implies "+p)
    p match {
...
object Permission {
  lazy val wildcardToken  = GardenConfig.permissionWilcardToken.vend
  lazy val partDivider    = GardenConfig.permissionPartDivider.vend
  lazy val subpartDivider = GardenConfig.permissionSubpartDivider.vend
